document.addEventListener('DOMContentLoaded',()=>{
  ...
  /* Партнёры: показываем секцию только при наличии реальных файлов */
  const partnersSection=document.getElementById('partners');
  if(partnersSection){
    const partnersTrack=partnersSection.querySelector('.partners-track');
    const partnersSource=partnersSection.querySelector('[data-partners-source]');
    const partnerImages=partnersSource
      ? [...partnersSource.querySelectorAll('img')].filter(img=>{
          const src=img.getAttribute('src')||'';
          return /(?:^|\/)assets\/partners\/.+\.(svg|png|webp)(?:[?#].*)?$/i.test(src);
        })
      : [];

    if(!partnersTrack||!partnersSource||!partnerImages.length){
      ...
      let partnersResizeTimer=0;
      function buildPartnerLoop(){
        partnersTrack.classList.remove('is-running');
        partnersTrack.querySelectorAll('.partners-group:not([data-partners-source])').forEach(group=>group.remove());

        let groups=1;
        while((partnersTrack.scrollWidth<innerWidth*2||groups<2)&&groups<12){
          const clone=partnersSource.cloneNode(true);
          clone.removeAttribute('data-partners-source');
        ...
        if(!reduced)requestAnimationFrame(()=>partnersTrack.classList.add('is-running'));
      }

      Promise.all(partnerImages.map(img=>{
        if(img.complete)return Promise.resolve();
        return new Promise(resolve=>{
          img.addEventListener('load',resolve,{once:true});
          img.addEventListener('error',resolve,{once:true});
        });
      })).then(buildPartnerLoop);

      window.addEventListener('resize',()=>{
        clearTimeout(partnersResizeTimer);
        partnersResizeTimer=setTimeout(buildPartnerLoop,220);
      },{passive:true});
    }
  }
